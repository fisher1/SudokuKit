...
class DigitRecognizerTraining:
    ...
    def __init__(self):
        self.training_pics = [Pic(pic_name=str(pathlib.Path(__file__).parent.absolute()) + "\\training.png", contour_dimension_from_h=21, contour_dimension_to_h=38)]

    def train(self):
        """Method to train digits"""
        # Loop all images to train
        for training_pic in self.training_pics:
            print(training_pic.pic_name)
            ...
            for cnt in contours:
                if cv2.contourArea(cnt) > (training_pic.contour_dimension_to_h * 2):
                    ...
                    if h > training_pic.contour_dimension_from_h and h < training_pic.contour_dimension_to_h:
                        im_copy = cv2.imread(training_pic.pic_name)
                        cv2.rectangle(im_copy, (x - 1, y - 1), (x + 1 + w, y + 1 + h), (0, 0, 255), 1)
                        roi = thresh[y:y + h, x:x + w]
                        roismall = cv2.resize(roi, (10, 10))
                        cv2.imshow('Training: Enter digits displayed in the red rectangle!', im_copy)
                        key = cv2.waitKey(0)
